...
function formatToolCall(
  ...
  case "bash": {
    ...
    const preview = ellipsis(command, 60);
  ...
  default: {
    ...
    const preview = ellipsis(argsStr, 50);
...
function getDisplayItems(messages: Message[]): DisplayItem[] {
  return messages
    ...
    .flatMap(part => {
      switch (part.type) {
      case "text":
        return [{ type: "text", text: part.text } as DisplayItem];
      case "toolCall":
...
const TaskItem = ty.Object({
  agent: ty.String({ description: "Name of the agent to invoke" }),
  task: ty.String({ description: "Task to delegate to the agent" }),
  cwd: ty.Optional(ty.String({ description: "Working directory for the agent process" })),
});

const ChainItem = ty.Object({
  agent: ty.String({ description: "Name of the agent to invoke" }),
  task: ty.String({ description: "Task with optional {previous} placeholder for prior output" }),
  cwd: ty.Optional(ty.String({ description: "Working directory for the agent process" })),
});
...
const SubagentParams = ty.Object({
  // TODO: same as TaskItem and ChainItem
  agent: ty.Optional(ty.String({ description: "Name of the agent to invoke (for single mode)" })),
  task: ty.Optional(ty.String({ description: "Task to delegate (for single mode)" })),
  cwd: ty.Optional(ty.String({ description: "Working directory for the agent process (single mode)" })),

  tasks: ty.Optional(ty.Array(TaskItem, { description: "Array of {agent, task} for parallel execution" })),
  chain: ty.Optional(ty.Array(ChainItem, { description: "Array of {agent, task} for sequential execution" })),
  agentScope: ty.Optional(AgentScopeSchema),
  confirmProjectAgents: ty.Optional(
    ty.Boolean({ description: "Prompt before running project-local agents. Default: true.", default: true }),
  ),
});

export default function (pi: ExtensionAPI) {
  return; // conflicts with npm:subagents
  pi.registerTool({
    name: "subagent",
    label: "Subagent",
    description: [
      ...
      `Default agent scope is "user" (from ${userScopeDir}).`,
      `To enable project-local agents in ${projectScopeDir}, set agentScope: "both" (or "project").`,
    ].join(" "),
      ...
      if (params.chain && params.chain.length > 0) {
        ...
        for (const [i, step] of params.chain.entries()) {
          ...
          const result = await runSingleAgent(
            ...
            signal,
            chainUpdate,
            results => makeDetails("chain", results),
          );
          results.push(result);
        ...
        const results = await mapWithConcurrencyLimit(params.tasks, MAX_CONCURRENCY, async (t, index) => {
          const result = await runSingleAgent(
            ...
            results => makeDetails("parallel", results),
          );
        ...
        const summaries = results.map(r => {
          ...
          const preview = (output.slice(0, 100) + (output.length > 100 ? "..." : "")) || "(no output)";
          const status = r.exitCode === 0 ? "completed" : "failed";
          return `[${r.agent}] ${status}: ${preview}`;
        });
        return {
          content: [
            {
    ...
    renderCall(args, theme, _context) {
      ...
      const text = (() => {if (args.chain && args.chain.length > 0) {
        return theme.fg("toolTitle", theme.bold("subagent ")) +
          theme.fg("accent", `chain (${args.chain.length} steps)`) +
          theme.fg("muted", ` [${scope}]`) +
          [...args.chain.slice(3).entries().map(([i, step]) => {
            // Clean up {previous} placeholder for display
            const cleanTask = step.task.replace(/\{previous\}/g, "").trim();
            const preview = ellipsis(cleanTask, 40);
            return "\n  " +
              theme.fg("muted", `${i + 1}.`) +
              " " +
        ...
        return theme.fg("toolTitle", theme.bold("subagent ")) +
          ...
          theme.fg("muted", ` [${scope}]`) +
          args.tasks.slice(0, 3).map(t => `\n  ${theme.fg("accent", t.agent)}${theme.fg("dim", ` ${ellipsis(t.task, 40)}`)}`).join("") +
          (args.tasks.length > 3 ? `\n  ${theme.fg("muted", `... +${args.tasks.length - 3} more`)}` : "");
      } else {
        const agentName = args.agent || "...";
        const preview = args.task ? ellipsis(args.task, 60) : "...";
        return theme.fg("toolTitle", theme.bold("subagent ")) +
    ...
    renderResult(result, { expanded }, theme, _context) {
      ...
      const renderDisplayItems = (items: DisplayItem[], limit?: number) => {
        ...
        const text = (skipped > 0 ? theme.fg("muted", `... ${skipped} earlier items\n`) : "") +
          toShow.map(item => {
            if (item.type === "text") {
      ...
      function aggregateUsage(results: SingleResult[]) {
        return results.map(r => r.usage).reduce((acc, u) => ({
      ...
      if (details.mode === "chain") {
        ...
        if (expanded) {
          ...
          for (const r of details.results) {
            ...
            // Show tool calls
            for (const item of displayItems.filter(item => item.type === "toolCall")) {
              container.addChild(new Text(
                theme.fg("muted", "→ ") + formatToolCall(item.name, item.args, theme.fg.bind(theme)),
              0, 0));
            }
