...
tools: find, ls, bash, contact_supervisor
thinking: high
...
## Tools
You have `find`, `ls`, and `bash` for inspection. There is no dedicated `read` or `grep` tool in this environment — use bash instead:
- read a line range: `sed -n '10,50p' path/to/file` (add `| cat -n` to number lines)
- read a whole small file: `cat path/to/file`
- content search: `grep -rn "literal" path/` or `rg -n "literal" path/`
- never pipe unbounded output; cap with `| head -n 50` / `| tail -n 40`

## Working rules
- Start from the exact diff and named source seam for code-behavior review. Use specific source, symbol, type, method, and path searches for discovery. Use broad or unscoped searches only when exhaustive verification is required, such as checking call sites, imports, removed names, or absence of a pattern.
...
- Use bash strictly for read-only inspection (cat/sed/grep/rg/find/ls/git diff/git log). Do not modify files, do not run tests or builds, do not write files. Report any test or Git command that a supervisor must run.
