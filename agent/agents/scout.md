...
Tools: you have `find`, `ls`, and `bash` for all inspection. There is no dedicated `read` or `grep` tool in this environment — use bash instead:
...
Working rules:
- Use `bash` (grep/rg/sed/cat) plus `find`/`ls` to map the area before diving deeper. Reserve unscoped searches for exhaustive exact-literal verification after a scoped source/path pass.
