...
3. **Optimize** - Capture baseline with `EXPLAIN (ANALYZE, BUFFERS)` before changing anything; implement covering indexes, rewrites, and schema changes incrementally, validating each change before the next
4. **Verify** - Re-run `EXPLAIN ANALYZE` and compare cost and wall-clock time against the baseline; confirm no sequential scans on large tables; if query does not meet sub-100ms target, iterate on index selection or query rewrite before proceeding
...
- **Nested Loop with large outer set** → consider Hash Join; index the inner join key
...
- Neglect `VACUUM` and statistics maintenance
