...
-- MySQL 8.0+: actual execution statistics
EXPLAIN ANALYZE
...
-- Good: single join with pre-aggregated CTE
...
-- Bad: implicit join syntax, filter applied after join
...
-- Good: explicit joins, filter early
SELECT p.name, c.name, s.stock
FROM categories c
...
```sql
-- Materialize reused/expensive CTEs
...
-- Force inline for single-use CTEs so the planner optimizes through them
...
```sql
-- Bad: multiple correlated subqueries
SELECT
    o.id,
    o.total,
...
-- Good: single window function scan
SELECT
    id,
    total,
...
```sql
-- Pre-aggregate large-cardinality groups before final aggregation
WITH daily_stats AS (
    SELECT
...
```sql
-- Bad: large OFFSET scans and discards skipped rows
SELECT * FROM products
...
-- MySQL: handler statistics (compare before/after a query)
SHOW STATUS LIKE 'Handler%';
FLUSH STATUS;
-- run your query
SHOW STATUS LIKE 'Handler%';

-- MySQL: slow query candidates from performance_schema
