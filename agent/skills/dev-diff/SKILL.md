...
Before working in local-files mode, read [references/local-files-mode.md](references/local-files-mode.md) — it holds the full mode rules: what helpers are safe, how to build and check the recap, and how to report the result.
...
Before authoring any recap, read [references/block-authoring.md](references/block-authoring.md). It holds the complete
Diff → Block Mapping (which block carries each kind of change), the full block
reference (exact tags + prop schemas — call `get-plan-blocks` at authoring time,
do not author from memory), and the before/after comparison patterns
(`columns`, `before-after`, and friends) that are the recap's center of gravity.
## Bidirectional Loop
