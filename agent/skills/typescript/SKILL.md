...
- **Use TypeScript only when the project already uses it.** Do not introduce TypeScript into plain JavaScript codebases.
- **`strict: true` always** when working in an existing TypeScript project. It enables `strictNullChecks`,
...
`./references/generics.md` for distributive behavior, `infer`, modifier removal, and key remapping.
...
- **`in` operator and optional properties:** if `Human` has `swim?`, `"swim" in animal` narrows to `Fish | Human`, not
  just `Fish`.
- **Truthiness narrowing pitfall:** fails on `""`, `0`, `NaN`, `false`. Prefer explicit null checks over truthiness when
...
- **Never coerce enums to booleans.** Compare explicitly: `level !== Level.NONE`, not `!!level`. Numeric enum value `0`
  is falsy.
- **Never mix numeric and string members** in the same enum.
- **Use enums over unions when** you need a runtime object (iteration, lookup), a namespace for related constants, or
  reverse mapping.
...
- **When overloads are necessary**, put specific signatures before general ones — TypeScript picks the first matching
  overload.
...
- **Only touch `tsconfig.json` in projects that already use TypeScript.** Do not add TS config to JavaScript projects.
- **`strict: true` always** in TypeScript projects. Also enable `noUncheckedIndexedAccess` and `noImplicitOverride`.
