...
Recommended sample counts per scenario (quick local check: 6, pre-merge: 10, small changes: 20-30, noisy CI: 20+) and the statistical reasoning behind them: see [Benchmark Methodology](./methodology.md).

**Never "retry until significant"** - rerunning benchmarks until `~` goes away introduces selection bias (p-hacking). At α=0.05, expect ~5% of benchmarks to randomly report significance with no real change (false positives). This is normal - don't chase them. Run once with a high `-count` and accept the result.
...
The general pitfall list (single runs, laptop battery, background load, cross-machine comparison, retry-until-significant, wide confidence intervals, unequal sample sizes) lives in [Benchmark Methodology](./methodology.md). Benchstat-specific ones:
...
See [Go CI Benchmark Tools](./go-ci-tools.md) for integrating benchstat comparisons into CI pipelines with benchdiff, cob, and gobenchdata.
