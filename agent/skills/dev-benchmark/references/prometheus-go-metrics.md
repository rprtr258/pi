...
**Performance note:** the default `go_memstats_*` metrics internally call `runtime.ReadMemStats()`, which triggers a short stop-the-world pause. The runtime/metrics-based collector below is cheaper and safer for concurrent reads - prefer it in high-throughput services.
...
## Investigation Deep-Dive Queries

Use these during temporary investigation sessions with a reduced scrape interval. Each query includes what to look for and what the result means. Session setup: see [investigation-session.md](./investigation-session.md).
