...
def generate_spec(
  skill_path: Path,
  model: str = "AgentLLM-big",
  max_retries: int = 2,
) -> ComplianceSpec:
  """Generate a compliance spec from a skill/rule file.

  Calls pi -p with the spec_generator prompt, parses YAML output.
  ...
  for attempt in range(max_retries + 1):
    ...
    result = subprocess.run(
      ["pi", "-p", prompt, "--model", model, "--no-session"],
      capture_output=True,
      text=True,
      timeout=120,
    )
    ...
    if result.returncode != 0:
      raise RuntimeError(f"pi failed: {result.stderr}")
