...
def _mock_compliant_classification(spec: ComplianceSpec, trace: list[ObservationEvent], model: str = "haiku"):  # noqa: ARG001
...
def _mock_noncompliant_classification(spec: ComplianceSpec, trace: list[ObservationEvent], model: str = "haiku"):
...
def _mock_empty_classification(spec: ComplianceSpec, trace: list[ObservationEvent], model: str = "haiku") -> dict[str, list[int]]:
  return {}


class TestGradeCompliant:
  @patch("scripts.grader.classify_events", side_effect=_mock_compliant_classification)
  def test_returns_compliance_result(self, mock_cls: Any, tdd_spec: ComplianceSpec, compliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, compliant_trace)
    assert isinstance(result, ComplianceResult)

  @patch("scripts.grader.classify_events", side_effect=_mock_compliant_classification)
  def test_full_compliance(self, mock_cls: Any, tdd_spec: ComplianceSpec, compliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, compliant_trace)
    assert result.compliance_rate == 1.0

  @patch("scripts.grader.classify_events", side_effect=_mock_compliant_classification)
  def test_all_required_steps_detected(self, mock_cls: Any, tdd_spec: ComplianceSpec, compliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, compliant_trace)
  ...
  @patch("scripts.grader.classify_events", side_effect=_mock_compliant_classification)
  def test_optional_step_detected(self, mock_cls: Any, tdd_spec: ComplianceSpec, compliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, compliant_trace)
    refactor = next(s for s in result.steps if s.step_id == "refactor")
    assert refactor.detected is True

  @patch("scripts.grader.classify_events", side_effect=_mock_compliant_classification)
  def test_no_hook_promotion_recommended(self, mock_cls: Any, tdd_spec: ComplianceSpec, compliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, compliant_trace)
    assert result.recommend_hook_promotion is False

  @patch("scripts.grader.classify_events", side_effect=_mock_compliant_classification)
  def test_step_evidence_not_empty(self, mock_cls: Any, tdd_spec: ComplianceSpec, compliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, compliant_trace)
...
class TestGradeNoncompliant:
  ...
  def test_low_compliance(self, mock_cls: Any, tdd_spec: ComplianceSpec, noncompliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, noncompliant_trace)
    assert result.compliance_rate < 1.0

  @patch("scripts.grader.classify_events", side_effect=_mock_noncompliant_classification)
  def test_write_test_fails_ordering(self, mock_cls: Any, tdd_spec: ComplianceSpec, noncompliant_trace: list[ObservationEvent]) -> None:
  ...
  @patch("scripts.grader.classify_events", side_effect=_mock_noncompliant_classification)
  def test_run_test_red_not_detected(self, mock_cls: Any, tdd_spec: ComplianceSpec, noncompliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, noncompliant_trace)
    run_red = next(s for s in result.steps if s.step_id == "run_test_red")
    assert run_red.detected is False

  @patch("scripts.grader.classify_events", side_effect=_mock_noncompliant_classification)
  def test_hook_promotion_recommended(self, mock_cls: Any, tdd_spec: ComplianceSpec, noncompliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, noncompliant_trace)
    assert result.recommend_hook_promotion is True

  @patch("scripts.grader.classify_events", side_effect=_mock_noncompliant_classification)
  def test_failure_reasons_present(self, mock_cls: Any, tdd_spec: ComplianceSpec, noncompliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, noncompliant_trace)
...
class TestGradeEdgeCases:
  ...
  def test_empty_trace(self, mock_cls: Any, tdd_spec: ComplianceSpec) -> None:
    result = grade(tdd_spec, [])
    assert result.compliance_rate == 0.0
    assert result.recommend_hook_promotion is True

  @patch("scripts.grader.classify_events", side_effect=_mock_compliant_classification)
  def test_compliance_rate_is_ratio_of_required_only(self, mock_cls: Any, tdd_spec: ComplianceSpec, compliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, compliant_trace)
    assert result.compliance_rate == 1.0

  @patch("scripts.grader.classify_events", side_effect=_mock_compliant_classification)
  def test_spec_id_in_result(self, mock_cls: Any, tdd_spec: ComplianceSpec, compliant_trace: list[ObservationEvent]) -> None:
    result = grade(tdd_spec, compliant_trace)
    assert result.spec_id == "tdd-workflow"

  @patch("scripts.grader.classify_events")
  def test_after_step_can_reference_later_declared_spec_step(self, mock_cls: Any) -> None:
    spec = ComplianceSpec(
