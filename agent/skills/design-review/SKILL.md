...
3. **Capture screenshots of the running application.**
   ...
   1. **Playwright MCP (preferred).** Check if the `playwright` MCP server is available. If it is, use it — it gives you precise control over viewport sizing, full-page captures, and file naming.
   2. **Chrome DevTools MCP (second choice).** If Playwright MCP is not available, use the `chrome-devtools` MCP server screenshot tools instead.
