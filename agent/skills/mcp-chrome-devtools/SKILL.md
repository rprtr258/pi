...
**Page selection**: Tools operate on the currently selected page. Use `list_pages` to see available pages, then `select_page` to switch context. Most tools require a `pageId` argument (from `list_pages`); browser restarts invalidate old ids — re-run `list_pages` after any restart instead of reusing stale ids.
